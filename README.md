...
- 📝 **Sınav modu** — **Sınıf + öğrenci + konu** seç ve değerlendir. Konu **öğrenci bazlıdır**
  (her öğrencinin konusu kaydedilir). Altta sınıf ilerleme listesi (satıra tıklayarak öğrenci
  seçilir, kim değerlendirildi görünür); değerlendirme sonrası otomatik olarak sıradaki bekleyen
  öğrenciye geçer. Sınıf **özet tablosu** (öğrenci + konu + kriter kırılımı + sınıf ortalaması) ve
